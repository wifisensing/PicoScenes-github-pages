...
## 16.1. 从我们这里采购USRP设备的优势：放心、省钱、省事 {#advantages-of-choosing-us}
...
## 16.2. 我们推荐的USRP型号 {#recommended-models}
...
[NI USRP X410](https://www.ettus.com/all-products/usrp-x410/) 是USRP系统目前性能最强的设备，它可在1-7200 MHz范围，最大4通道400 MHz采样([Listening to 40/80/160/320 MHz Bandwidth Channels](scenarios.html#6212-listening-to-4080160320-mhz-bandwidth-channels), [Transmitting 40/80/160/320 MHz bandwidth 802.11a/g/n/ac/ax/be Format Frames](scenarios.html#62212-transmitting-4080160320-mhz-bandwidth-80211agnacaxbe-format-frames))。为支持最大1.6 GSPS的基带信号传输，X410配备了100 GbE连接。时钟同步方面，它内置GPSDO，也支持输出时钟信号也支持输入外部时钟信号([Clock Synchronization across Multiple USRP Devices](scenarios.html#62161-clock-synchronization-across-multiple-usrp-devices))，支持USRP多设备组合功能([Combining Multiple USRP devices](scenarios.html#62162-combining-multiple-usrp-devices))，支持Radar mode([Wi-Fi Radar (802.11bf Mono-Static Sensing Mode) with 802.11a/g/n/ac/ax/be Frame Format](scenarios.html#623-wi-fi-radar-80211bf-mono-static-sensing-mode-with-80211agnacaxbe-frame-format))，可实现多机、多通道条件下的时钟同步。
...
## 16.3. 其它要考虑的因素 {#other-things-to-consider}

### 16.3.1. 我需要买几台设备？

您在确定设备采购台数时，可考虑以下因素：

1. Radar Mode([Wi-Fi Radar (802.11bf Mono-Static Sensing Mode) with 802.11a/g/n/ac/ax/be Frame Format](scenarios.html#623-wi-fi-radar-80211bf-mono-static-sensing-mode-with-80211agnacaxbe-frame-format))是对USRP新用户非常友好的模式，仅需要一台NI USRP B210就可以进行感知研究；
...
## 16.4. 报价与支付方式 {#partner-store}

### 16.4.1. USRP产品的厂商指导价 v.s 我们的报价规则 
...
## 16.5. 亲身经历：不要选择山寨USRP，会后悔 {#why-not-choose-piracy}

...
{% include tip.html content="如果您不想费劲地看英文，请开启浏览器的翻译功能，省脑子。" %}
...
{% include tip.html content="PicoScenes sets many Rx parameters by default, such as using the *RX_CBW_20* preset, using the Tx/Rx antenna port, using the normalized 0.65 Rx gain, etc. See [Rx Gain Control: Manual GC and AGC](#6214-rx-gain-control-manual-gc-and-agc) for Rx Gain control." %}
...
{% include tip.html content="You can refer to [PicoScenes Presets](presets.html) for a full list of presets." %}
...
2. **Specifying the normalized Rx gain**: To set the Rx gain using a normalized value, you can use the `--rx-gain` option followed by the desired normalized gain value. For example:
    ...
    {% include tip.html content="PicoScenes sets `--rx-gain` to 0.65 by default." %}
...
2. **Single USRP Device - Dual/Multi-Channel Rx with Dual 10GbE connections**
    ...
    {% include tip.html content="You can follow the guides below to setup dual 10GbE connections for the X3x0 and N3x0 series. 1. X3x0 Series: [Using Dual 10 Gigabit Ethernet on the USRP X300/X310](https://kb.ettus.com/Using_Dual_10_Gigabit_Ethernet_on_the_USRP_X300/X310)
...
{% include tip.html content="PicoScenes uses 802.11n format for packet injection by default." %}
...
{% include tip.html content="You can refer to the [PicoScenes Presets](presets.html) for a full list of presets." %}
...
{% include tip.html content="PicoScenes specifies ``--txpower 0.7`` by default." %}
...
{% include tip.html content="Due to the cyclic shift delay (CSD) requirement of the 802.11 standard, even for a 1-STS frame, the signals transmitted on each Tx channel are different and cyclically delayed among the antennas." %}
...
{% include tip.html content="See how we [implement the Radar mode in less than 30 lines of code](https://github.com/wifisensing/PicoScenes-PDK/-/commit/ee0242a6837dabfc259a26236799ddd9b1eb893c)?" %}
...
{% include warning.html content="It is essential to comply with the RF spectrum regulations of your country/location. PicoScenes platform is a research-purpose software. You are responsible for ensuring compliance with all applicable laws." %}
...
{% include tip.html content="By default, PicoScenes sets ``--rate`` to 20 MHz. If the ``--preset`` option is used, it will override the defaults. If both ``--preset`` and ``--rate`` are explicitly provided, the ``--rate`` option will take precedence over ``--preset``." %}
...
{% include tip.html content="In-baseband Digital Resampling is a computationally intensive task. It may lower performance and overall throughput." %}
...
{% include tip.html content="For a more comprehensive explanation of this multi-line format, refer to the [PicoScenes Command Line Interface](parameters.html#picoscenes-command-line-interface) section." %}
...
{% include tip.html content="PicoScenes MATLAB Toolbox Core (PMT-Core) also provides a decoder for .bbsignals files. You can simply drag and drop the .bbsignals file into the MATLAB Command Window to open it." %}
...
{% include note.html content="To save signals back into a .bbsignals file, you can utilize the ``writeBBSignals`` commands provided by PMT-Core." %}
...
{% include note.html content="High Doppler-format is an optional feature of 802.11ax standard. AX210/AX200 doesn’t support this mode." %}
...
{% include note.html content="ESS is an optional feature of the 802.11n standard. It is supported by QCA9300 and IWL5300, but not by AX210/AX200." %}
...
{% include tip.html content="The two frequencies specified in ``--freq`` can be any two frequencies supported by the hardware, providing more research flexibility." %}

{% include note.html content="The 320 MHz sampling rate, along with *In-baseband Digital Resampling*, is extremely CPU intensive. Users should expect a very high packet loss rate." %}
...
{% include tip.html content="For a more comprehensive explanation of this multi-line format, please refer to the [PicoScenes Command Line Interface](parameters.html#picoscenes-command-line-interface) section." %}

...
如前所述，PicoScenes团队很荣幸得到NI的认可与支持，荣获 **NI USRP授权销售资格以及优质集成商资格**。在这个页面，我们将详细介绍以下几方面内容：

- [从我们这里采购USRP设备的优势](#advantages_of_choosing_us)
- [我们推荐的USRP型号](#recommended_models) 
- [其它要考虑的因素](#other_things_to_consider)
- [报价与支付方式](#partner_store)
- [亲身经历：不要选择山寨USRP](#why_not_choose_shanzhai)

## 16.1. 从我们这里采购USRP设备的优势：放心、省钱、省事 {#advantages_of_choosing_us}
...
## 16.2. 我们推荐的USRP型号 {#recommended_models}
...
### 16.2.2. NI USRP X310母板搭配双UBX-160子板
...
### 16.2.4. NI USRP X310母板搭配双TwinRx子板
...
[NI USRP X410](https://www.ettus.com/all-products/usrp-x410/) 是USRP系统目前性能最强的设备，它可在1-7200 MHz范围，最大4通道400 MHz采样([Listening to 40/80/160/320 MHz Bandwidth Channels](scenarios.md#6212-listening-to-4080160320-mhz-bandwidth-channels), [Transmitting 40/80/160/320 MHz bandwidth 802.11a/g/n/ac/ax/be Format Frames](scenarios.md#62212-transmitting-4080160320-mhz-bandwidth-80211agnacaxbe-format-frames))。为支持最大1.6 GSPS的基带信号传输，X410配备了100 GbE连接。时钟同步方面，它内置GPSDO，也支持输出时钟信号也支持输入外部时钟信号([Clock Synchronization across Multiple USRP Devices](scenarios.md#62161-clock-synchronization-across-multiple-usrp-devices))，支持USRP多设备组合功能([Combining Multiple USRP devices](scenarios.md#62162-combining-multiple-usrp-devices))，支持Radar mode([Wi-Fi Radar (802.11bf Mono-Static Sensing Mode) with 802.11a/g/n/ac/ax/be Frame Format](scenarios.md#623-wi-fi-radar-80211bf-mono-static-sensing-mode-with-80211agnacaxbe-frame-format))，可实现多机、多通道条件下的时钟同步。

### 16.2.7. NI OctoClock-G
...
## 16.3. 其它要考虑的因素 {#other_things_to_consider}   

### 16.3.1. 我需要买几台设备？ {#how_many_devices_do_i_need}  

您在确定设备采购台数时，可考虑以下因素：

1. Radar Mode([Wi-Fi Radar (802.11bf Mono-Static Sensing Mode) with 802.11a/g/n/ac/ax/be Frame Format](scenarios.md#623-wi-fi-radar-80211bf-mono-static-sensing-mode-with-80211agnacaxbe-frame-format))是对USRP新用户非常友好的模式，仅需要一台NI USRP B210就可以进行感知研究；
...
### 16.3.2. 您需要对货期有预估 {#estimate_the_delivery_time}  

NI USRP系列完全由NI在马来西亚或匈牙利的工厂按订单生产，从生产、寄出、清关最终交付您手上，大约需要4-6周时间（一般来说4周概率高）。如果您有一些关键的项目时间节点，请提前规划。

### 16.3.3. 强烈推荐您采购NI原厂延保服务 {#strongly_recommend_you_to_purchase_the_ni_original_warranty_service}  
...
## 16.4. 报价与支付方式 {#pricing_and_payment_methods}

### 16.4.1. USRP产品的厂商指导价 v.s 我们的报价规则 {#the_manufacturer_recommended_price_vs_our_pricing_rules}  
...
### 16.4.2. 我们的报价（2024年5月） {#our_pricing_2024_may}  
...
### 16.4.3. 支付方式 {#payment_methods} 

请加微信咨询购买: jiangzhiping；我们支持学校或机构通过采购合同方式购买，可开具电子发票，支持对公转账。

## 16.5. 亲身经历：不要选择山寨USRP，会后悔 {#personal_experience_do_not_choose_fake_usrp_you_will_regret_it}   

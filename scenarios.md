...
## 6.1. Before Getting Started: Some Fundamentals {#before_getting_started_some_fundamentals}
...
### 6.1.1. Device Naming {#device_naming}   

In order to mult-frontend operation, we devise an simply device naming protocol, which is elaborated in the follow section.

#### 6.1.1.1. Device Naming for Commercial Wi-Fi NICs {#device_naming_for_commercial_wi_fi_nics}
...
#### 6.1.1.2. Device Naming for SDR {#device_naming_for_sdr}
...
##### 6.1.1.2.1. Device Naming for NI USRP {#device_naming_for_ni_usrp}
...
##### 6.1.1.2.2. Device Naming for HackRF One {#device_naming_for_hackrf_one}
...
##### 6.1.1.2.3. Device Naming for Virtual SDR {#device_naming_for_virtual_sdr}
...
### 6.1.2. Basic Facts of Wi-Fi Channelization {#basic_facts_of_wi_fi_channelization}
...
## 6.2. ISAC Research using NI USRP or HackRF One SDR {#isac_research_using_ni_usrp_or_hackrf_one_sdr}
...
### 6.2.1. Listening to Wi-Fi Traffic and Measuring CSI for 802.11a/g/n/ac/ax/be-Format Frame {#listening_to_wi_fi_traffic_and_measuring_csi_for_80211agnacaxbe_format_frame}

#### 6.2.1.1. Listening to 20 MHz Bandwidth Channels {#listening_to_20_mhz_bandwidth_channels}
...
#### 6.2.1.2. Listening to 40/80/160/320 MHz Bandwidth Channels {#listening_to_4080160320_mhz_bandwidth_channels}
...
#### 6.2.1.3. Antenna Selection (Only for NI USRP Device) {#antenna_selection_only_for_ni_usrp_device}
...
#### 6.2.1.4. Rx Gain Control: Manual GC and AGC {#rx_gain_control_manual_gc_and_agc}
...
#### 6.2.1.5. Multi-Channel Rx by Single NI USRP Device {#multi_channel_rx_by_single_ni_usrp_device}
...
#### 6.2.1.6. Multi-Channel Rx by Multiple NI USRP Devices {#multi_channel_rx_by_multiple_ni_usrp_devices}
...
##### 6.2.1.6.1. Clock Synchronization across Multiple USRP Devices {#clock_synchronization_across_multiple_usrp_devices}   
...
##### 6.2.1.6.2. Combining Multiple USRP Devices {#combining_multiple_usrp_devices}
...
##### 6.2.1.6.3. Combining Multiple USRP Devices plus Dual-10GbE Connection {#combining_multiple_usrp_devices_plus_dual_10gbe_connection}
...
### 6.2.2. Transmitting 802.11a/g/n/ac/ax/be Protocol Frames using SDR Devices {#transmitting_80211agnacaxbe_protocol_frames_using_sdr_devices}

#### 6.2.2.1. Single-Device Tx with Rich Low-Level Controls {#single_device_tx_with_rich_low_level_controls}
...
##### 6.2.2.1.1. Transmitting 20 MHz Bandwidth 802.11n Format Frames {#transmitting_20_mhz_bandwidth_80211n_format_frames}
...
##### 6.2.2.1.2. Transmitting 40/80/160/320 MHz Bandwidth 802.11a/g/n/ac/ax/be Format Frames {#transmitting_4080160320_mhz_bandwidth_80211agnacaxbe_format_frames}
...
##### 6.2.2.1.3. Tx Gain Control {#tx_gain_control}
...
#### 6.2.2.2. Multi-Channel (RF Chain) and MIMO Tx with NI USRP Devices {#multi_channel_rf_chain_and_mimo_tx_with_ni_usrp_devices}
...
##### 6.2.2.2.1. Multi-Channel (RF Chain) Tx for 1-STS Frame with NI USRP Device {#multi_channel_rf_chain_tx_for_1_sts_frame_with_ni_usrp_device}
...
##### 6.2.2.2.2. Multi-Channel (RF Chain) Tx for MIMO Frame with NI USRP Device {#multi_channel_rf_chain_tx_for_mimo_frame_with_ni_usrp_device}
...
### 6.2.3. Wi-Fi Radar (802.11bf Mono-Static Sensing Mode) with 802.11a/g/n/ac/ax/be Frame Format {#wi_fi_radar_80211bf_mono_static_sensing_mode_with_80211agnacaxbe_frame_format}
...
#### 6.2.3.1. Wi-Fi Radar (802.11bf Mono-Static Sensing) SISO mode {#wi_fi_radar_80211bf_mono_static_sensing_siso_mode}
...
#### 6.2.3.2. Wi-Fi Radar (802.11bf Mono-Static Sensing) SIMO mode {#wi_fi_radar_80211bf_mono_static_sensing_siso_mode}
...
#### 6.2.3.3. Wi-Fi Radar (802.11bf Mono-Static Sensing) MIMO Mode {#wi_fi_radar_80211bf_mono_static_sensing_mimo_mode}
...
- We recommend to wait a few seconds before transmission, as the ``--delayed-start 3`` option indicates to wait 3 seconds before transmission.
 ...
 #### 6.2.3.4. Wi-Fi Radar (802.11bf Mono-Static Sensing) MIMO Mode using Multiple USRP Devices {#wi_fi_radar_80211bf_mono_static_sensing_mimo_mode_using_multiple_usrp_devices}
...
### 6.2.4. Transmission, Reception, and CSI Measurement with Non-Standard Channel and Bandwidth {#transmission_reception_and_csi_measurement_with_non_standard_channel_and_bandwidth}
...
#### 6.2.4.1. Changing Baseband Bandwidth (Sampling Rate) with NI USRP B2x0 Series {#changing_baseband_bandwidth_sampling_rate_with_ni_usrp_b2x0_series}
...
#### 6.2.4.2. Non-Standard Tx/Rx with NI USRP N2x0/X3x0/N3x0 Series {#non_standard_txrx_with_ni_usrp_n2x0x3x0n3x0_series}
...
### 6.2.5. Concurrent Multi-SDR Operation on a Single Computer {#concurrent_multi_sdr_operation_on_a_single_computer}
...
### 6.2.6. Advanced Features {#advanced_features}

#### 6.2.6.1. Signal Recording and Replaying (Both Tx and Rx Ends) {#signal_recording_and_replaying_both_tx_and_rx_ends}
...
#### 6.2.6.2. Multiple CSI Measurements per Frame {#multiple_csi_measurements_per_frame}
...
##### 6.2.6.2.1. 802.11ax High Doppler-Format Frames {#80211ax_high_doppler_format_frames}
...
##### 6.2.6.2.2. 802.11n Extra Spatial Sounding (ESS) Frames {#80211n_extra_spatial_sounding_ess_frames}
...
#### 6.2.6.3. Channel Impairment Simulation {#channel_impairment_simulation}

> **Todo**: need to be verified.

#### 6.2.6.4. Dual-Channel Spectrum Splitting and Stitching (Experimental) {#dual_channel_spectrum_splitting_and_stitching_experimental}
...
#### 6.2.6.5. Multi-Thread Rx Decoding (Experimental) {#multi_thread_rx_decoding_experimental}
...
## 6.3. CSI Measurement using AX210/AX200 NICs {#csi_measurement_using_ax210ax200_nics} 
...
### 6.3.1. CSI Measurement from Associated Wi-Fi AP {#csi_measurement_from_associated_wi_fi_ap}
...
### 6.3.2. Fully-Passive CSI Measurement in Monitor Mode {#fully_passive_csi_measurement_in_monitor_mode}
...
### 6.3.3. Packet Injection-Based CSI Measurement (Tx with 802.11a/g/n/ac/ax Format and 20/40/80/160 MHz CBW) {#packet_injection_based_csi_measurement_tx_with_80211agnacax_format_and_204080160_mhz_cbw}
...
### 6.3.4. Packet Injection with MCS Setting and Antenna Selection {#packet_injection_with_mcs_setting_and_antenna_selection}
...
### 6.3.5. Specifying Channel and Bandwidth in Real-time {#specifying_channel_and_bandwidth_in_real_time}
...
### 6.3.6. Concurrent Multi-NIC Operation on a Single Computer {#concurrent_multi_nic_operation_on_a_single_computer}
...
## 6.4. CSI Measurement using QCA9300 and IWL5300 NICs {#csi_measurement_using_qca9300_and_iwl5300_nics}
...
### 6.4.1 CSI Measurement from Associated AP by IWL5300 {#csi_measurement_from_associated_ap_by_iwl5300}    
...
### 6.4.2. Packet Injection based CSI Measurement {#packet_injection_based_csi_measurement}
...
### 6.4.3. Specifying Tx and Rx Chains {#specifying_tx_and_rx_chains}
...
### 6.4.4. Concurrent Multi-NIC Operation for QCA9300 and IWL5300 {#concurrent_multi_nic_operation_for_qca9300_and_iwl5300}
...
### 6.4.5. QCA9300 Operating with Non-Standard Channel, Bandwidth, and Manual Rx Gain {#qca9300_operating_with_non_standard_channel_bandwidth_and_manual_rx_gain}
...
## 6.5. Interoperability among SDR and COTS NICs {#interoperability_among_sdr_and_cots_nics}    
